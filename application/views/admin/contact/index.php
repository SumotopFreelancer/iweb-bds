  ...
  <div class="box">
    ...
    <div class="box-body table-responsive no-padding mailbox-messages">
      <table class="table table-hover">
        <form method="GET" action="<?= admin_url('contact') ?>">
          <tr>
            <th style="width: 30%">
            ...
            <th class="pull-right">
              ...
              <a href="<?= admin_url('contact') ?>" title="Làm mới" class="btn btn-default" type="submit"><i class="fa fa-refresh"></i></a>
            </th>
          </tr>
        </form>
      </table>
        ...
        <?php foreach ($list as $key => $row) : ?>
          <tr class='row_<?= $row->id ?>'>
            ...
            <td>
              ...
              <p><b>Ghi chú: </b><?= $row->content ?></p>
            </td>
            <td><?= $row->content ?></td>
            <td><?= get_date_admin($row->created) ?></td>
            <td class="b-active">
              <a href="<?= admin_url('contact/delete/' . $row->id) ?>" class="btn btn-sm btn-social-icon btn-danger btn_del_one" title="Xóa"><i class="fa fa-trash-o fa-fw"></i></a>
            </td>
          </tr>
        <?php endforeach; ?>
      </table>
    ...
    <div class="box-footer clearfix">
      <button type="button" class="btn btn-sm btn-danger btn_del_all" url="<?= admin_url('contact/del_all') ?>">Xóa hết</button>
      <?= $phantrang ?>
    </div>
  </div>
</section>