...
<section class="content-header">
  <h1>Trang
    <a href="<?= admin_url('pages/add') ?>" class="btn btn-success btn-flat">
...
<section class="content">
  <div class="box">
      ...
      <table class="table table-hover cus_text_mid">
        ...
        <?php foreach ($list as $key => $row) : ?>
          <tr class='row_<?= $row->id ?>'>
            ...
            <td>
              <a class="text-black line-1" target="_blank" href="<?= site_url(_pg . '/' . $row->url) ?>"><?= $row->name ?></a>
            </td>
            ...
            <td class="b-active">
              <a href="<?= admin_url('pages/edit/' . $row->id) ?>" class="btn btn-sm btn-social-icon btn-warning" title="Chỉnh sửa"><i class="fa fa-edit fa-fw"></i></a>
              <a href="<?= admin_url('pages/delete/' . $row->id) ?>" class="btn btn-sm btn-social-icon btn-danger btn_del_one" title="Xóa"><i class="fa fa-trash-o fa-fw"></i></a>
            </td>
          </tr>
        <?php endforeach; ?>
      </table>
    ...
    <div class="box-footer clearfix">
      <button type="button" class="btn btn-sm btn-danger btn_del_all" url="<?= admin_url('pages/del_all') ?>">Xóa hết</button>
      <?= $phantrang ?>
    </div>
  </div>
</section>
