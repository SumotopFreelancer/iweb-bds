...
<section class="content-header">
  <h1>Danh mục <small>Hình ảnh</small>
    <a href="<?= admin_url('catalogimage/add') ?>" class="btn btn-success btn-flat">
...
<section class="content">
  <div class="box">
      ...
      <table class="table table-hover cus_text_mid">
        ...
        foreach ($catalog as $row) : ?>
          <tr class='row_<?= $row->id ?>'>
            ...
            <td>
              <a class="text-black" target="_blank" href="<?= base_url(_cim . '/' . $row->url) ?>"><b><?= $row->name ?></b></a>
            </td>
            ...
            <?php /*
              ...
              */ ?>
            <td class="b-active">
              <a href="<?= admin_url('catalogimage/edit/' . $row->id) ?>" class="btn btn-sm btn-social-icon btn-warning" title="Chỉnh sửa"><i class="fa fa-edit fa-fw"></i></a>
              <a href="<?= admin_url('catalogimage/delete/' . $row->id) ?>" class="btn btn-sm btn-social-icon btn-danger btn_del_one" title="Xóa"><i class="fa fa-trash-o fa-fw"></i></a>
            </td>
          </tr>
          <?php if (count($this->catalogimage_model->menucon_admin($row->id)) > 0) : ?>
            <?php $j = $i + 1;
            foreach ($this->catalogimage_model->menucon_admin($row->id, check_sort($setadmin->sort_catalog_image)) as $lv1) : ?>
              <tr class='row_<?= $lv1->id ?>'>
                <td class="text-center"><input type="checkbox" name="id[]" value="<?= $lv1->id; ?>"></td>
                <td class="cus_td_50"><?= $j ?></td>
                <td>
                  <a class="text-black" target="_blank" href="<?= base_url(_cim . '/' . $lv1->url) ?>">--|<?= $lv1->name ?></a>
                </td>
                ...
                */ ?>
                <td class="b-active">
                  <a href="<?= admin_url('catalogimage/edit/' . $lv1->id) ?>" class="btn btn-sm btn-social-icon btn-warning" title="Chỉnh sửa"><i class="fa fa-edit fa-fw"></i></a>
                  <a href="<?= admin_url('catalogimage/delete/' . $lv1->id) ?>" class="btn btn-sm btn-social-icon btn-danger btn_del_one" title="Xóa"><i class="fa fa-trash-o fa-fw"></i></a>
                </td>
              </tr>
              <?php if (count($this->catalogimage_model->menucon_admin($lv1->id)) > 0) : ?>
                <?php $k = $j + 1;
                foreach ($this->catalogimage_model->menucon_admin($lv1->id, check_sort($setadmin->sort_catalog_image)) as $lv2) : ?>
                  <tr class='row_<?= $lv2->id ?>'>
                    <td class="text-center"><input type="checkbox" name="id[]" value="<?= $lv2->id; ?>"></td>
                    <td class="cus_td_50"><?= $k ?></td>
                    <td>
                      <a class="text-black" target="_blank" href="<?= base_url(_cim . '/' . $lv2->url) ?>">--|--|<?= $lv2->name ?></a>
                    </td>
                  ...
                  */ ?>
                    <td class="b-active">
                      <a href="<?= admin_url('catalogimage/edit/' . $lv2->id) ?>" class="btn btn-sm btn-social-icon btn-warning" title="Chỉnh sửa"><i class="fa fa-edit fa-fw"></i></a>
                      <a href="<?= admin_url('catalogimage/delete/' . $lv2->id) ?>" class="btn btn-sm btn-social-icon btn-danger btn_del_one" title="Xóa"><i class="fa fa-trash-o fa-fw"></i></a>
                    </td>
                  </tr>
                  <?php if (count($this->catalogimage_model->menucon_admin($lv2->id)) > 0) : ?>
                    <?php $l = $k + 1;
                    foreach ($this->catalogimage_model->menucon_admin($lv2->id, check_sort($setadmin->sort_catalog_image)) as $lv3) : ?>
                      <tr class='row_<?= $lv3->id ?>'>
                        <td class="text-center"><input type="checkbox" name="id[]" value="<?= $lv3->id; ?>"></td>
                        <td class="cus_td_50"><?= $l ?></td>
                        <td>
                          <a class="text-black" target="_blank" href="<?= base_url(_cim . '/' . $lv3->url) ?>">--|--|--|<?= $lv3->name ?></a>
                        </td>
                    ...
                    */ ?>
                        <td class="b-active">
                          <a href="<?= admin_url('catalogimage/edit/' . $lv3->id) ?>" class="btn btn-sm btn-social-icon btn-warning" title="Chỉnh sửa"><i class="fa fa-edit fa-fw"></i></a>
                          <a href="<?= admin_url('catalogimage/delete/' . $lv3->id) ?>" class="btn btn-sm btn-social-icon btn-danger btn_del_one" title="Xóa"><i class="fa fa-trash-o fa-fw"></i></a>
                        </td>
                      </tr>
                    <?php $l++;
                      $k++;
    ...
    <div class="box-footer clearfix">
      <button type="button" class="btn btn-sm btn-danger btn_del_all" url="<?= admin_url('catalogimage/del_all') ?>">Xóa hết</button>
    </div>
  </div>
</section>
<script type="text/javascript">
  $(".statusajax").click(function() {
    ...
    $.ajax({
      type: "post",
      url: "<?= admin_url('catalogimage/status') ?>",
      data: {
        id: id
      },
      beforeSend: function() {
  ...
  $(".homeajax").click(function() {
    ...
    $.ajax({
      type: "post",
      url: "<?= admin_url('catalogimage/home') ?>",
      data: {
        id: id
      },
      beforeSend: function() {
