...
class Tagsproduct extends MY_Controller
{
	...
	{
		// Pagination
		$config = $this->adminpagination->config($this->tagsproduct_model->get_total(), admin_url('tagsproduct'), 40, $_GET, admin_url('tagsproduct'), 3);
		...
		$this->data['list'] = $this->tagsproduct_model->get_list($input);
		$this->data['total_rows'] = $config['total_rows'];
		// Load view
		$this->data['temp'] = 'admin/tagsproduct/index';
	...
	function check_tags_name()
	{
		...
		if ($check && $this->tagsproduct_model->check_name_tags($name)) {
	...
	function add()
	{
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->tagsproduct_model->create($data)) {
				...
				redirect(admin_url('tagsproduct'));
			}
		}
		// Load view
		$this->data['temp'] = 'admin/tagsproduct/add';
		$this->load->view('admin/main', $this->data);
	}
	function edit()
	{
		$id = intval($this->uri->rsegment('3'));
		$info = $this->tagsproduct_model->get_info($id);
		if (!$info) {
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				$where = ['url' => $url];
				if ($this->tagsproduct_model->check_exists($where)) {
					$info_tags = $this->tagsproduct_model->get_info_rule($where);
					if ($info_tags->id != $id) {
						$input = [];
						$input['where'] = ['name' => $name];
						$num = $this->tagsproduct_model->get_total($input);
				...
				if ($this->tagsproduct_model->update($id, $data)) {
				...
				if ($this->input->post('cus_btn_save') == 'Lưu lại') {
					redirect(admin_url('tagsproduct/edit/' . $id));
				} else {
					redirect(admin_url('tagsproduct'));
				}
			}
		}
		$this->data['temp'] = 'admin/tagsproduct/edit';
		$this->load->view('admin/main', $this->data);
	}
	// Xóa
	function delete()
	...
	private function _del($id, $ajax = TRUE)
	{
		...
		if ($this->tagsproduct_model->delete($id)) {
			echo '<span class="id_delete hidden">' . $id . '</span>';
		}
		// xóa Tag
		$input = [];
		$input = ['tag_id' => $id];
		$this->tagsproductproducts_model->del_rule($input);
	}
}
