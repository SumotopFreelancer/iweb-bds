...
class Catalognew extends My_Controller
{
	...
	function index()
	{
		$this->data['total_rows'] = $this->catalognew_model->get_total();
		// Load view
	...
	function add()
	{
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				$url = get_url_add(chuyenurl($this->input->post('url', TRUE)), $name, 'catalognew_model');
				$data = [
					'name' => $name,
					'url' => $url,
	...
	function edit()
	{
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				$name = trim($this->input->post('name', TRUE));
				$url = get_url_edit(chuyenurl($this->input->post('url', TRUE)), $name, 'catalognew_model', $id);
				$data = [
					'name' => $name,
					'url' => $url,
					'intro' => $this->input->post('intro'),
				...
				if ($this->catalognew_model->update($id, $data)) {
					// Sửa trong bảng menu
					$where = ['id_type' => $id, 'type' => 'catalognew'];
	...
	private function _del($id, $ajax = TRUE)
	{
		...
		if (count($this->catalognew_model->menucon_admin($info->id)) > 0) {
			if ($ajax) {
		...
		$ip_where = ['catalog_id' => $id];
		$check_exists = $this->newscatalognew_model->check_exists($ip_where);
		...
		if ($this->catalognew_model->delete($id)) {
			// Xóa catalog_id trong bảng new_catalognew
			$where_del = ['catalog_id' => $id];
			$this->newscatalognew_model->del_rule($where_del);
